...
int centroid(int fsize, unsigned short *frame, double *vals)
{
	...
	*/
	...
	for (i = supxcent, xmaxpix = ymaxpix = maxpix = 0; i <= supxcent + 1; i++)
		for (j = supycent; j <= supycent + 1; j++)
		{
			...
			maxpix = (*(frame + i + (j * fsize)) > maxpix) ? *(frame + i + (j * fsize)) : maxpix;
		}
	...
	*/
	...
	asq = (2.0 * (mxxnorm + myynorm)) + (2.0 * sqrt(((mxxnorm - myynorm) * (mxxnorm - myynorm)) + (4.0 * mxynorm * mxynorm)));
	bsq = (2.0 * (mxxnorm + myynorm)) - (2.0 * sqrt(((mxxnorm - myynorm) * (mxxnorm - myynorm)) + (4.0 * mxynorm * mxynorm)));
	...
	*/
	...
	/*
	Find the fwhm along the centroid row and columns
	*/

	for (i = ix; i < fsize; i++)
		if ((*(frame + i + (iy * fsize)) - sky) < hm )
		{
			fwhmx = 2.0 * ((double) (abs(i - ix)));
			break;
		}
